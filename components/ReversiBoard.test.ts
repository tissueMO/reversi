  ...
  it('初期石が4つ配置されている', () => {
    ...
    positions.forEach(({ row, col, color }) => {
      const cell = wrapper.find(
        `.board-row:nth-child(${row + 1}) .board-cell:nth-child(${col + 1})`,
      );
...
describe('石を置く操作', () => {
  it('有効なマスに石を置くと手番が交代する', async () => {
    ...
    expect(
      player1Score.classes().includes('current-turn') ||
      player2Score.classes().includes('current-turn'),
    ).toBe(true);
  });
  it('無効なマスをクリックしても石が置かれない', async () => {
    ...
    // 無効なマス（初期配置の石がある場所）
    const cell = wrapper.find(
      '.board-row:nth-child(4) .board-cell:nth-child(4)',
    );
...
describe('デバッグ・CPU機能', () => {
  it('skipToEndGameで終局2手前になる', async () => {
    ...
    const empty = wrapper
      .findAll('.board-cell')
      .filter(
        (cell) => !cell.find('.piece-black') && !cell.find('.piece-white'),
      );
    expect(empty.length).toBeLessThanOrEqual(2);
  });
  it('skipToEndGameで終局5手前になる', async () => {
    const wrapper = mountBoard();
    await wrapper.vm.skipToEndGame(5);
    const empty = wrapper
      .findAll('.board-cell')
      .filter(
        (cell) => !cell.find('.piece-black') && !cell.find('.piece-white'),
  ...
  it('skipToEndGameで黒が多くなるようにできる', async () => {
    const wrapper = mountBoard();
    await wrapper.vm.skipToEndGame(2, BLACK);
  ...
  it('skipToEndGameで白が多くなるようにできる', async () => {
    const wrapper = mountBoard();
    await wrapper.vm.skipToEndGame(2, WHITE);
    const black = wrapper.findAll('.piece-black').length;
    const white = wrapper.findAll('.piece-white').length;
...
describe('クラッカー（紙吹雪）エフェクトの表示', () => {
  ...
  it('プレイヤー対プレイヤー：引き分けの場合はクラッカーが表示されない', async () => {
    ...
    const wrapper = mountBoard({ gameMode: 'twoPlayers' });
    Math.random = originalRandom;
    await wrapper.vm.skipToEndGame(2);
    await wrapper.vm.forceGameEnd();
    await new Promise((r) => setTimeout(r, 600));
    await wrapper.vm.$nextTick();
  ...
  it('プレイヤー対CPU：プレイヤーが勝つとクラッカーが表示される', async () => {
    ...
    Math.random = originalRandom;
    await wrapper.vm.skipToEndGame(2, BLACK); // 黒が多い盤面
    await wrapper.vm.forceGameEnd();
    await new Promise((r) => setTimeout(r, 600));
    await wrapper.vm.$nextTick();
    expect(wrapper.find('.confetti-container').exists()).toBe(true);
